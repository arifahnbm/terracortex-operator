...
export default function EngineRpmAlert({ rpm, status, pressure, advisory }: EngineRpmAlertProps) {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Alert Box */}
      <div className={`rounded-xl p-4 flex gap-4 shadow-md transition-colors duration-300 border ${
        isOverload ? 'bg-red-50 border-red-200' : 
        status === 'warn' ? 'bg-orange-50 border-orange-200' : 
        'bg-white border-gray-300'
      }`}>
        <div className="flex-shrink-0 mt-1">
          {isOverload || status === 'warn' ? (
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isOverload ? "text-red-500" : "text-orange-500"}>
        ...
        <div className="flex flex-col">
          {isOverload ? (
            ...
            <>
              <h3 className="font-bold text-sm mb-1 uppercase text-orange-800">WARNING</h3>
              <p className="text-sm text-orange-900 leading-snug">{advisory || "Check engine metrics."}</p>
            </>
          ) : (
            <>
